#!/usr/bin/python
# get channel_id list from subscription list
# YouTube の登録チャンネルリストから channel_id を取得する
...
while True: #nextPageToken が存在する(次ページが存在する)かぎりループ 最大50件なので
    search_list_request = youtube.subscriptions().list(
        part='snippet',
        maxResults=50,
        mine=True,
        pageToken=pToken,
...
# 全チャンネルの channelId を表示
f = open('youtube-channel_id-list.txt', 'a')
for idx in range(len(channel_ids)):
    #print(idx, channel_ids[idx])
    cid_line = channel_ids[idx] + '\n'
    f.write(cid_line)

f.close()

f = open('youtube-channel_id-list.txt', 'r')
...
# add_subscription(youtube, 'UCG8wOav28pu6mT9NXRlWohw')
